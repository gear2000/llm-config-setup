...
Every `instructions.md` the leader authors is a document a fresh worker consumes cold, so write it through the `writing-for-agents` skill. State one narrow goal, what is in scope, what is out of scope, the authoritative plan and repository context, and checkable and demanding completion conditions. Tell the worker to return `blocked` with the exact decision needed if the stage requires a wider scope or a design change. The brief must let the worker understand the overall agreed work, not just the mechanical step. Inline what every path needs, point at what only some paths reach, and state the target behavior positively. Protocol-mandated verbatim blocks (result template, phone book, scope leash) are pasted unchanged; the writing rules govern the prose around them.
