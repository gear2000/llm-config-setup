...
- Stage-4 is the TUI-performed apply, recorded from the receipt (runner `tui-apply`, receipt path as evidence) — the leader hires no stage-4 worker in an iac phase. The TUI always re-plans fresh at apply time (see /tui-control) rather than applying `plan.bin`, so what runs is never an opaque saved-plan artifact. The one exception is the variant for very long applies, where no human is live at the terminal to reconfirm a fresh plan: a worker order with `operation: apply`, `requires_apply: true`, and the `plan_artifact`/`approval` blocks (the same SHA-bound contract the direct controller enforces) applies the exact artifact the human already approved, because an unattended apply needs an immutable plan to bind to instead of a live re-plan.
