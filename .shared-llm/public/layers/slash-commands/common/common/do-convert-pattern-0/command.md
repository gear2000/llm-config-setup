...
5. Recommend documents from the actual pool. Default bundled choices while GPT 5.6 Luna is discounted are `phase-low-Luna` for easy, `phase-medium-Luna` for medium, and `phase-high-Luna` for hard. Other bundled choices are `phase-low` for easy, `phase-medium-sonnet` or `phase-medium-chatgpt-5.5` for medium, and `phase-high-Astra`, `phase-high-Fable` or `phase-high-Sol` for hard. These are recommendations, not a closed registry or automatic model selection. Custom pools may use different names. Obtain human agreement on exact documents, especially medium/hard variants. Read their requirements rather than duplicating model rosters here.
