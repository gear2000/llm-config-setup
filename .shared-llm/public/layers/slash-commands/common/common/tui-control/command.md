...
3. Validate the installed Herdr command surface (`herdr workspace list/create`, `herdr pane list/split/run/read/close`, `herdr pane wait-output`, `herdr agent wait`). Adapt only after validating any local syntax differences.
