...
Validate the installed Herdr command surface before launch (the documented baseline is `herdr agent start/get/wait`, `herdr pane split/get/process-info/run/read/close`). The Recruiter splits a pane off the cockpit, then issues one `agent start --kind --pane` request with direct argv; it never types a launch command into a shell. If the local Herdr version exposes different syntax, adapt only after validating it. A malformed order or result is fail-loud: the Recruiter refuses to hire on a bad order; the leader treats a missing or malformed result as a `blocked` stage.
