...
- injects the stage instructions, route details, worktree branch, deterministic merge timing, the non-delegation rule, and the specialist phone book — the VERBATIM output of `just specialist-hub roster` (merged roster + exact consult mechanics + the mandatory-consult rule) — into `instructions.md`;
