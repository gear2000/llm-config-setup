...
3. Grill the user with Planish by default, using the `grill-me` skill's design-tree method: work the plan as a design tree in frontier rounds — each round asks every question whose prerequisites are settled, numbered, with a recommended answer; facts are looked up, decisions go to the user. Render each round per the shared Planish HTML Grill Contract at `.shared-llm/public/llm/common/common/planish-html-grill-contract.md`: static annotatable HTML, sticky notes, Copy Feedback, pasted feedback, no in-page submit, and no plain chat questionnaire unless the user explicitly asks for a terminal fallback.
4. Resolve design only when the work requires it.
   - For a one-way-door design decision (new module boundary, shared API, schema, event, or ownership change), sketch before prose: produce at least two structurally distinct design sketches — the caller's usage written first, then the type sketch, function signatures, and module map with `not implemented` bodies — compare them, and record in the plan which sketch won and why. Whole-shape alternatives, not point fixes inside one shape. Skip sketching for mechanical work whose shape is already concrete; a second sketch round over a settled design is over-engineering. Create or update `design.md`/ADRs when the plan changes service, package, trust, persistence, deployment, shared API, event, schema, security, state, concurrency, migration, rollback, performance, availability, or parallel-phase architecture. If existing docs answer the design question, cite them and continue.
...
- After human approval, default execution is Flow 1: a human-started Claude Code pane runs `/hil --plan <plan.md> --offering <id> --effort <effort>`. `/hil` is Claude-only (not installed on Pi or Codex). A `/do-plan` approval hands that plan to such a Claude Code HIL pane rather than invoking `/hil` inside Pi. Do not start `/cc-full`, `/do-full`, `/cc-implement`, `/do-implement`, or `just run-start` unless the human explicitly asks.
