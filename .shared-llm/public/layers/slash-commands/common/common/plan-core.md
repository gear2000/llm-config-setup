...
Resolve one plan directory before writing anything by running `.shared-llm/public/llm/common/common/planish_resolve.py --topic <topic> [--dir <path>]` and reading its `plan_dir` JSON result; do not reproduce this selection in model prose. The resolver's precedence is: explicit `--dir <path>`, then `$PLANISH_DIR`, then the nearest `.planish.yaml` found by walking upward from the current directory, then `/tmp/planish/{date}/{slug}`. A configured `dir:` must be a non-empty string or planning fails loudly; never silently fall back from a malformed config. Relative configured paths are resolved from the config file's directory. Expand `{date}`, `{slug}`, `{type}` (`plan`), and `{n}` (the next sibling version). `$PLANISH_HOST` or the config's `host:` controls the review URL only.
