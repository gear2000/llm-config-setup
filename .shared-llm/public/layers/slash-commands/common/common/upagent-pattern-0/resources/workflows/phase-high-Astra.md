...
```text
GPT 6 Astra low codes -> GPT 5.6 Sol high adversarial review
  -> required checks + pass -> next phase
```

## Models

- Coder: offering `pi-gpt-6-astra`; model `openai-codex/gpt-6-astra`; harness `pi`; effort `low`.
...
2. Hire Astra for the assigned scope and original acceptance criteria. Hire a fresh GPT 5.6 Sol adversary against the resulting candidate. Return findings to Astra within the approved retry bound. Exhaustion means stop and ask the human.
