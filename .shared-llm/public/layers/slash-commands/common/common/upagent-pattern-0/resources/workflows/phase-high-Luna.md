...
```text
GPT 5.6 Luna max codes -> Fable 5.1 adversarial review
  -> repair/review -> required checks + pass -> next phase
```

Default hard-tier document while GPT 5.6 Luna is discounted. The reviewer is always the opposite model family from the coder.
