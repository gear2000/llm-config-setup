...
Run one phase as the Herdr-native **phase leader**. This command is sent to a top-left cockpit pane by `/herdr-run`, once per phase. The leader runs the phase's stages by placing a work ORDER per stage to the UpAgent Recruiter — never by spawning a native or nested subagent — applies stage-level backtracking and the stage-try budget ladder, maintains `phase-status.md`, and writes `phase-result.json` (the source of truth the TUI reads).
