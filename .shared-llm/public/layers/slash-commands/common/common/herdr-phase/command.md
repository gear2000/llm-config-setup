...
3. Validate this phase's route entry:
   ...
   - each profile's `model` is the harness-native id shape — claude: alias or full name (paired with `effort`); pi: `provider/id[:thinking]` (the `:thinking` suffix is pi's effort). A `provider/…` model on a claude profile, or a bare id on a pi profile, is a route error — fail loud, do not guess a translation;
