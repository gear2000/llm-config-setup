...
5. Start one standing run-level result watcher for the whole run tree. This is a tiny Haiku/low-effort helper, not a Recruiter order and not one per phase. Its only job is mechanical polling: scan `phases/*/phase-result.json` for existence/mtime changes, parse the verdict field, and notify the TUI when it sees a terminal verdict (`passed`, `failed`, or `blocked`) that is not already recorded in `run-status.md`. It reads no code, makes no judgment, never advances the run itself, and stays silent when nothing changed.
...
3. **Wait for the completion event, always bounded.** Use `herdr wait output <leader-pane> --match "PHASE_RESULT: <phase-id>" --timeout <ms>` and parse its `verdict=<passed|failed|blocked>` marker, while the standing run-level watcher independently watches for `phases/<phase-id>/phase-result.json` to reach a terminal verdict. Never use `agent-status=done`: a leader can become idle between its own turns while a Recruiter order is still running. If the watcher reports the file first, treat that as the wake-up event and read the file immediately. On timeout, read and validate `phases/<phase-id>/phase-result.json`; proceed only if valid, otherwise mark the phase `blocked` and stop for the human.
...
6. Append a `run-status.md` line for the phase outcome (phase id, pass number, verdict, and any `revisit`) before acting on the verdict. This makes the run-level watcher idempotent: a later scan sees that terminal result already logged and stays quiet. On every start/pass/fail/backtrack — or hourly if unchanged — delegate a minimal static HTML snapshot to a small disposable, non-stage helper. Give it only `run-status.md`, `phase-status.md`, and `phase-result.json` paths; it writes a per-phase done/in-progress/blocked, current-stage, one-line-summary snapshot and returns only its path. Reuse the existing Planish HTML/CSS baseline (`planish-html-grill-contract.md` and `toolkits/`); resurface via file-send/Artifact, never inline.
