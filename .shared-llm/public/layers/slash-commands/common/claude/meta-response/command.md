...
A **small done-ping** to the meta-orchestrator hub. After `/meta-run` has written `results.md` to
...
detect completion by watching `results.md` appear on disk — `/meta-response` only lets it react sooner
...
```
/meta-response --hub <hub-json-or-url> --output <results.md> [--name <worker-name>] [--msg <msg_id>]
```
...
- `--hub <hub-json-or-url>` — the hub's discovery JSON path (the file with `url` + `pid`, default
  ...
  `/meta-connect` does.
- `--output <results.md>` — absolute path to the results file `/meta-run` just wrote. You read its
...
- `--name <worker-name>` — optional. This worker's name/address on the hub (the one used at
  `/meta-connect`). Used as the `from` when sending back to the brain.
...
- **`--hub` missing** → STOP: `Usage: /meta-response --hub <hub-json-or-url> --output <results.md> [--name <worker-name>] [--msg <msg_id>]`.
...
exactly as `/meta-connect`. Missing file / no `url` → STOP:
...
`ERROR: <output> does not start with a PHASE_RESULT: line — /meta-run did not write it correctly.`
...
5. **The results file is the source of truth; `/meta-response` is the accelerator** — say so in the
   report (Step 5).
