...
```
/cc-plan-and-grill [--team] [--route-phases] [--interactive] [--docs] <title>
```
...
Use this flag when the plan will be executed AFK via `/cc-loop --afk`. Without size tags, `/cc-loop --afk` refuses to start.
...
- **WITHOUT `--route-phases`**: phases must NOT have `**Size**:` lines. The plain grill produces a clean plan that `/cc-loop --interactive` and `/cc-implement` consume.

**Downstream effect:** `/cc-loop --afk` refuses plans without size tags. `/cc-loop --interactive` accepts both.
...
5. **Last-phase live-deploy gate** — before finalizing, scan the last phase's verification list. At least one entry must match a live-deploy pattern (`git push`, `curl https://`, `aws lambda invoke`, `aws logs tail`, `terraform apply`, CI/pipeline triggers, playwright against live URL). If none found, tell the user and re-enter the grill until the gate passes.
...
small if ALL of:
  ...
  - no live-deploy verification commands (git push, curl https://,
    aws lambda invoke, terraform apply, CI/pipeline triggers, playwright
    against live URL)
...
**Dispatch a plan-writer subagent** with research.md as input and the user's stated goal. Same prompt template as `/cc-plan` (see that skill's Step 3 for the full prompt + required frontmatter + required sections, including the `## Team` section spec).
...
- **Team roster** — does this need 1 or 2 workers? Is the deploy nontrivial enough to dispatch a `deployer`? Is the work risky enough to want a `plan-watchdog`? Does any phase need a specialist (`database`, `security`, `devops`)? Resolve this in the grill — it goes in the plan's `## Team` section that drives `/cc-implement`.
...
4. Without `--route-phases`: Tell the user: _"Plan finalized at `<path>`. Run `/cc-implement <path>/plan.md` when ready."_
   ...
   Then `/cc-loop --afk <phases_root>` to execute AFK.
   ```
