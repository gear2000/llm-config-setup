...
requester, the Python Recruiter, and the LLM management roles. Runtime adapters may execute
...
def request_identity(order: dict) -> str:
    """Return the stable durable identity for one logical order.
    ...
    digest = hashlib.sha256(
        f"{order_id}\0{Path(result_path).expanduser().absolute()}".encode()
    ).hexdigest()
...
def requester_address(order: dict) -> RequesterAddress:
    ...
    if raw is None:
        ...
        return RequesterAddress(
            requester_id=f"pane:{pane}", kind="herdr-agent", address=pane
        )
    ...
    if kind not in REQUESTER_KINDS:
        raise LifecycleError(
            f"requester.kind must be one of {', '.join(REQUESTER_KINDS)}"
        )
...
def parse_manager_decision(
    text: str, request_id: str, generation: int
) -> ManagerDecision:
    value = _json_object(text, "manager decision")
    if value.get("request_id") != request_id:
        raise LifecycleError(
            "manager decision request_id does not match the current request"
        )
    ...
    if decision not in MANAGER_DECISIONS:
        raise LifecycleError(
            f"manager decision must be one of {', '.join(MANAGER_DECISIONS)}"
        )
    message = _require_string(value.get("message"), "manager message")
    requested_changes = value.get("requested_changes", [])
    if not isinstance(requested_changes, list) or not all(
        isinstance(item, str) and item.strip() for item in requested_changes
    ):
        raise LifecycleError("requested_changes must be a list of non-empty strings")
    return ManagerDecision(
        request_id, current_generation, decision, message, tuple(requested_changes)
    )


def parse_check_assessment(
    text: str, request_id: str, generation: int
) -> CheckAssessment:
    value = _json_object(text, "check assessment")
    if value.get("request_id") != request_id:
        raise LifecycleError(
            "check assessment request_id does not match the current request"
        )
    current_generation = _require_generation(value.get("generation"), generation)
    assessment = _require_string(value.get("assessment"), "assessment")
    if assessment not in CHECK_ASSESSMENTS:
        raise LifecycleError(
            f"assessment must be one of {', '.join(CHECK_ASSESSMENTS)}"
        )
    ...
    if recommended_action not in RECOMMENDED_ACTIONS:
        raise LifecycleError(
            f"recommended_action must be one of {', '.join(RECOMMENDED_ACTIONS)}"
        )
...
def parse_intake_clerk_response(text: str) -> IntakeClerkResponse:
    ...
    if "order" in value:
        ...
        if not isinstance(notes, list) or not all(
            ...
            raise LifecycleError(
                "intake clerk notes must be a list of non-empty strings"
            )
    ...
    if unknown:
        raise LifecycleError(
            "refused intake response has unknown fields: " + ", ".join(sorted(unknown))
        )
...
def parse_requester_decision(
    ...
    if value.get("request_id") != request_id:
        raise LifecycleError(
            "requester decision request_id does not match the current request"
        )
    current_generation = _require_generation(value.get("generation"), generation)
    action = _require_string(value.get("action"), "requester action")
    if action not in REQUESTER_ACTIONS:
        raise LifecycleError(
            f"requester action must be one of {', '.join(REQUESTER_ACTIONS)}"
        )
    ...
    return RequesterDecision(
        request_id, current_generation, action, extension_ms, message
    )
...
class RequestMailbox:
    ...
    def publish(
        ...
        with _locked(self.lock):
            ...
            used = [
                ...
                for path in self.messages.glob("[0-9]*-*.json")
            ]
    ...
    def read_all(self) -> list[dict]:
        values = []
        for path in (
            sorted(self.messages.glob("*.json")) if self.messages.is_dir() else []
        ):
            try:
