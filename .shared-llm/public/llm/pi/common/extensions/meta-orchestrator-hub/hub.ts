/**
 * hub.ts — a standalone Pi extension: the `/meta-server:hub` command to start, inspect, and
 * stop the meta-orchestrator message hub.
 *
 * DELIBERATELY SEPARATE from index.ts (the brain). It registers exactly one
 * command, `/meta-server:hub`, and holds exactly one piece of state — the discovery JSON
 ...
 * failing here cannot affect meta-server:autorun; the two share nothing.
 *
 *   /meta-server:hub start [--json <path>]   start the hub (or just connect if one's there)
 *   /meta-server:hub status                  is THIS session's hub up?   (no flag needed)
 *   /meta-server:hub stop                    stop THIS session's hub      (no flag needed)
 *
...
/**
 * Check whether a hub is already up at `jsonPath` WITHOUT ever starting one. For `meta-server:run`,
 * which JOINS a hub the human already started and must never create or tear one down. Returns
 * `ok:true` (with the url/pid) when a healthy hub is there; `ok:false` with a fail-loud `error`
 * when the discovery file is missing or the recorded url doesn't answer /health. Never throws,
 * never spawns.
 */
export async function checkHubUp(jsonPath: string): Promise<HubStartResult> {
	const p = expandHome(jsonPath);
	const d = readDiscovery(p);
	if (!d) {
		return { ok: false, error: `no hub at ${p} — start one with \`meta-server:hub start\` (or use meta-server:autorun, which makes its own hub)` };
	}
	if (!(await healthy(d.url))) {
		return { ok: false, error: `stale hub discovery at ${p} — ${d.url} did not answer /health; start a hub with \`meta-server:hub start\`` };
	}
	return { ok: true, url: d.url, pid: d.pid, alreadyUp: true };
}

/** Stop the hub recorded at `jsonPath` (SIGTERM its pid; the hub removes its own JSON on SIGTERM).
 *  Never throws. Shared by `meta-server:hub stop` and the brain's auto-teardown on shutdown. */
...
export default function (pi: ExtensionAPI): void {
	...
	pi.registerCommand("meta-server:hub", {
		description: "Meta-orchestrator hub: /meta-server:hub start [--json <path>] · /meta-server:hub status · /meta-server:hub stop",
		handler: async (args: string, c: ExtensionContext): Promise<void> => {
			...
			try {
				...
				notify(`unknown: /meta-server:hub ${sub} — use start | status | stop`, "warning");
			} catch (err) {
				// Never let the handler throw — a thrown rejection could reach Pi's loop.
				notify(`/meta-server:hub error: ${err instanceof Error ? err.message : String(err)}`, "error");
			}
		},
	});
}
