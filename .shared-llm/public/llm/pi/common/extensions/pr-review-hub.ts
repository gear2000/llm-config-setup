...
const DEFAULT_MODEL =
	process.env.PR_REVIEW_MODEL || "openai-codex/gpt-5.6-luna:max";
...
export default function (pi: ExtensionAPI) {
	...
	const timers = new Map<
		string,
		{ fallback: NodeJS.Timeout; poll: NodeJS.Timeout }
	>();
	...
	function getReviewModel(req: any): string {
		return typeof req.model === "string" && req.model.trim()
			? req.model.trim()
			: DEFAULT_MODEL;
	}
	...
	function dispatch(
		...
		const timeoutSeconds =
			Number(req.timeout_seconds) || DEFAULT_TIMEOUT_SECONDS;
		...
		const content = [
			`[pr-review request_id=${requestId} review_key=${reviewKey}]`,
		];
			...
			.then((line) => {
				...
				if (!reviewKey) {
					reply(socket, {
						...
						error: "missing field: review_key",
					});
					return;
				}
				if (!MODES.includes(mode)) {
					reply(socket, {
						status: "error",
						request_id: requestId,
						error: `invalid mode: ${parsed.mode}`,
					});
					return;
				}
				...
				if (parsed.scope != null && !SCOPES.includes(String(parsed.scope))) {
					reply(socket, {
						status: "error",
						request_id: requestId,
						error: `invalid scope: ${parsed.scope}`,
					});
					return;
				}
				if (!fs.existsSync(parsed.repo_path)) {
					reply(socket, {
						status: "error",
						request_id: requestId,
						error: `repo_path not found: ${parsed.repo_path}`,
					});
					return;
				}
				if (!fs.existsSync(parsed.handoff_path)) {
					reply(socket, {
						status: "error",
						request_id: requestId,
						error: `handoff_path not found: ${parsed.handoff_path}`,
					});
					return;
				}
