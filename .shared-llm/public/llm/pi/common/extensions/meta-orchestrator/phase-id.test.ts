...
check("makePhaseId basic", makePhaseId("onboarding", 1) === "orch-onboarding-p1");
...
check("makeRunPhaseId basic", makeRunPhaseId("onboarding", "3", 7) === "orch-onboarding-p3-7");
...
check("makeRunPhaseId sanitizes slug", makeRunPhaseId("Add Project!", "1", 2) === "orch-add-project-p1-2");
{
	...
	const a = makeRunPhaseId("onboarding", "0", 1);
	const b = makeRunPhaseId("onboarding", "0", 5);
...
{
	const id = makePhaseId("onboarding", 2);
	check("log path", logPathFor("/r/logs", id) === "/r/logs/orch-onboarding-p2.log");
	check("socket path", socketPathFor("/r/sockets", id) === "/r/sockets/orch-onboarding-p2.sock");
	check("mcp config path", mcpConfigPathFor("/r/configs", id) === "/r/configs/orch-onboarding-p2.mcp.json");
}
