...
3. **Re-read that phase's section of the plan and decide the WORK agents. The agent list ALWAYS comes from you — never from the worker.** The list you pass is the phase's **work** roster only; you do **not** add a reviewer or evaluator to it — every phase automatically ends with a mandatory adversarial evaluation (an `adversarial-evaluator` subagent on the LLM profile the run's route assigns to the adversarial-audit stage — an explicit model + effort, never hardwired) that the worker runs itself, against the plan, after the work agents finish. So choose only the agents that do the phase's work; the gate is not yours to add. Exactly one of:
