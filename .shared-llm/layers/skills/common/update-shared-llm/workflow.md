...
live in the kit under `.shared-llm/layers/`; private layers live in the repo under
...
1. Create the layer file under `.shared-llm/layers/<type>/common/<name>/` (generic,
   kit) or `.shared-llm/this_repo/layers/<type>/this_repo/<name>.md` (private, repo).
2. Create the compose recipe under `.shared-llm/compose/<type>/<name>.yaml` (kit) or
