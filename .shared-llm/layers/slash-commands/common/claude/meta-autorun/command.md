...
```
/meta-autorun --plan <plan.md> --phase <instructions.md> --output <results.md> --hub <loc> [--team true]
```
...
- `--team true` — optional. Passed straight through to `/meta-run` so the work is done by a
...
Run **`/meta-connect <hub>`** with the `--hub` value (its JSON path or url). This reads the hub's
discovery JSON, health-checks `/health`, and registers this session as an agent. If it fails (no
hub, stale JSON, `/register` non-2xx), `/meta-connect` STOPs loud — surface that and stop; there is
no point doing the phase work for a brain you cannot reach. Note the worker name it registered as
(you pass it to `/meta-response` in step 3).

### Step 2 — Do the phase work and WRITE results.md (the mandatory disk write)

Run **`/meta-run --plan <plan.md> --phase <instructions.md> --output <results.md> [--team true]`**,
threading through the same `--plan`, `--phase`, `--output`, and `--team` you were given. `/meta-run`
...
Run **`/meta-response --hub <hub> --output <results.md> [--name <worker-name>] [--msg <msg_id>]`** — a
...
2. **Run the three steps in order: `/meta-connect` → `/meta-run` → `/meta-response`** (Steps 1-3). Do not
...
5. **This command is a thin conductor** — it invokes the three existing commands and threads the args
   through. It does not re-implement their logic (the hub handshake lives in `/meta-connect`, the work
   + gate + file write in `/meta-run`, the ping in `/meta-response`).
