...
```
/meta-run --plan <plan.md> --phase <instructions.md> --output <results.md> [--team true]
```
...
- **Any of `--plan` / `--phase` / `--output` missing** → print and STOP:
  ...
  ```
  Usage: /meta-run --plan <plan.md> --phase <instructions.md> --output <results.md> [--team true]
...
**ALWAYS** dispatch ONE more agent before the phase can pass: the **`adversarial-evaluator`**. This
is a **built-in mandatory gate**, not part of the team roster — it runs on **every** phase, you
never skip it, and you never substitute a work agent for it.

**The gate's LLM and effort are never hardwired — the route decides them.** The session dir (the
`/tmp/meta-orch/<session_name>/` root that holds `plan.md`) also holds the run's validated
`route.yaml`. Resolve this phase's `phases.phase-<N>.stages.stage-2-adversarial-audit.llm_profile`
— an explicit harness + model + effort — and dispatch the evaluator on exactly that profile:

- Profile harness `claude` → `Agent(subagent_type="adversarial-evaluator", model=<profile model>,
  …)`, stating the profile's effort expectation in the dispatch prompt.
- Any other profile harness (`pi`, `codex`, `cursor`, …) → run the audit through that harness's own
  launch path with the profile's model + effort. If this worker has no way to launch that harness,
  the gate is unresolvable — write `blocked` (Step 5) naming the missing harness. **Never silently
  substitute a different model or harness for the one the route names.**
- No `route.yaml` in the session dir (a legacy routeless launch) → dispatch the evaluator on your
  own session's model and record in `results.md` exactly which model + effort the gate ran on.

Dispatch it with: the phase's goal + work (from
...
6. **HUB-AGNOSTIC.** This command touches no hub — only files. Registering on the hub and pinging the
   brain are other commands' jobs (`/meta-connect`, `/meta-response`), chained by `/meta-autorun`.
