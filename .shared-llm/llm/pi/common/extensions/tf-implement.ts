      ...
      return {
        systemPrompt:
          ...
          (pendingIssues.length > 0
            ...
            : "STEP 1 — GRILL: Before writing anything, call the planish_grill tool with title, contextHtml, and a batch of clarifying infrastructure questions (regions, sizing, naming, dependencies, what already exists, ordering constraints). Give each question your recommended answer. Do NOT make a plain Q&A-only grill. Use the Planish visual ladder: simple → mermaid, medium → ascii, complex → visualHtml with .grill-fig/.flow/.flow-box. A diagram only when it genuinely helps — never for its own sake. Use the answers to inform the plan. Ask follow-ups by calling planish_grill again if needed.\n\n" +
