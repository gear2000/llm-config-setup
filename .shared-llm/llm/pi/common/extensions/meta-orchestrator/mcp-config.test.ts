...
// 3. claude argv: the exact flags the live CLI requires, in order, with the team prompt
{
	...
	check("argv starts with -p and the /team prompt", args[0] === "-p" && args[1] === "/team onboarding offboard then onboard", args.slice(0, 2).join(" "));
...
// 6. buildRunPhaseArgs: the brain's /run-phase prompt + the SAME ask_brain wiring as /team
{
	...
	check(
		...
		args[0] === "-p" && args[1] === "/run-phase plan=/tmp/plan.md phase=0 agents=code-review,deployer",
		args.slice(0, 2).join(" "),
	);
...
// 7. buildRunPhaseArgs: agents render as one comma-token (trimmed, no empties), model appended
{
	const args = buildRunPhaseArgs({
		planFile: "/p.md",
		phase: "3",
		agents: [" onboarding-agent ", "", "plan-watchdog"],
	...
	check(
		...
		args[1] === "/run-phase plan=/p.md phase=3 agents=onboarding-agent,plan-watchdog",
		args[1],
	);
