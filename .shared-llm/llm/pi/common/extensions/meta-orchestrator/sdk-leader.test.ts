...
function makeState(agents: string[] = ["code-review", "deployer", "onboarding-agent"]): LeaderState {
	return {
...
async function main() {
	...
	// ── case 8: buildSystemPrompt fills BOTH placeholders (every <PLAN_FILE> + the agent list) ──
	{
		...
		const out = buildSystemPrompt("/tmp/plan.md", ["code-review", "deployer"], promptPath);
		check("no <PLAN_FILE> placeholder remains", !out.includes("<PLAN_FILE>"), out);
		check("BOTH <PLAN_FILE> sites filled (input ref + launch template)", out.split("/tmp/plan.md").length === 3, out);
