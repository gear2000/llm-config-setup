...
Usage:
  ...
  meta-cc-client run-phase --plan <plan.md> --phase <N> \\
      --instructions-file <instructions.md> --route <route.yaml> --results <results.md> \\
      --session <run-name> --worker-type claude|pi|cursor [--model <model>] \\
      --mode subagents --hub-json <path> [--attempt <N>] [--timeout <ms>]
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (!tok.startsWith("--")) {
			throw new UsageError(
				`unexpected argument "${tok}" — expected --flag value pairs`,
			);
		}
...
async function cmdHubDown(argv: string[]): Promise<number> {
	...
	process.stdout.write(
		`stopped hub pid ${r.pid} (json ${expandHome(jsonPath)})\n`,
	);
	return 0;
}
...
async function cmdRunPhase(argv: string[]): Promise<number> {
	...
	const routeFile = path.resolve(need(flags, "route"));
	...
	if (
		workerTypeRaw !== "claude" &&
		workerTypeRaw !== "pi" &&
		workerTypeRaw !== "cursor"
	) {
		throw new UsageError(
			`--worker-type must be "claude", "pi", or "cursor", got "${workerTypeRaw}"`,
		);
	}
	...
	const model =
		workerType === "pi" || workerType === "cursor"
			? need(flags, "model")
			: (flags.get("model") ?? "");

	const modeRaw = need(flags, "mode");
	if (modeRaw !== "subagents") {
		throw new UsageError(
			`--mode must be "subagents" for synchronized meta execution; Claude team mode is not supported (got "${modeRaw}")`,
		);
	}
	const mode: "subagents" = "subagents";
	...
	if (attemptRaw !== undefined) {
		...
		if (!Number.isInteger(n) || n < 1) {
			throw new UsageError(
				`--attempt must be a positive integer (>= 1), got "${attemptRaw}"`,
			);
		}
		attempt = n;
	}
	...
	if (timeoutRaw !== undefined) {
		...
		if (!Number.isInteger(n) || n <= 0) {
			throw new UsageError(
				`--timeout must be a positive integer (ms), got "${timeoutRaw}"`,
			);
		}
		resultsTimeoutMs = n;
	}
	...
	const planContent = readFileLoud(planFile, "--plan");
	const routeContent = readFileLoud(routeFile, "--route");
	const runnableCheck = validateRunnable(planContent, routeContent);
	if (!runnableCheck.ok) {
		throw new UsageError(
			`runnable input check failed before worker launch\n${formatCheckResult("RUNNABLE_CHECK", runnableCheck)}`,
		);
	}
	...
	const fileBaseDir =
		process.env.META_ORCH_FILE_DIR || path.join(os.tmpdir(), "meta-orch");
	const sessionDir = path.join(fileBaseDir, sessionName);
	const logsDir = path.join(sessionDir, "logs");

	// TOCTOU guard: we just validated planContent/routeContent above, but the worker (spawned later by
	// hubRunPhase, possibly minutes from now) re-reads whatever is at planFile/routeFile at THAT time —
	// if either path is mutated between this check and the worker's read, the worker would run on
	// content that was never validated. Copy the validated bytes to immutable snapshots under the
	// session dir (mirroring index.ts's plan.md/route.yaml copy) and point the worker at those instead
	// of the original, externally-writable paths.
	const validatedPlanFile = path.join(sessionDir, "plan.md");
	const validatedRouteFile = path.join(sessionDir, "route.yaml");
	fs.mkdirSync(sessionDir, { recursive: true });
	fs.writeFileSync(validatedPlanFile, planContent, "utf-8");
	fs.writeFileSync(validatedRouteFile, routeContent, "utf-8");
	...
	const input: RunPhaseInput = {
		phaseId,
		runPhaseCommand: {
			planFile: validatedPlanFile,
			phase,
			instructions,
			instructionsPath,
			routeFile: validatedRouteFile,
			resultsPath,
	...
	process.stderr.write(
		`run-phase: verdict=${verdict ?? "none"} (log ${outcome.logPath})\n`,
	);
	return verdict === "passed" ? 0 : 1;
}

const KNOWN_RECORD_PROGRESS_FLAGS = new Set([
	"session-dir",
	"phase",
	"attempt",
	"results",
...
async function cmdRecordProgress(argv: string[]): Promise<number> {
	...
	const report =
		resultsRaw !== undefined
			? readFileLoud(path.resolve(resultsRaw), "--results")
			: undefined;
	...
	const planContent = readFileLoud(
		path.join(sessionDir, "plan.md"),
		"plan.md (under --session-dir)",
	);
...
async function main(): Promise<number> {
	...
	switch (sub) {
		...
		case undefined:
			throw new UsageError(
				"no subcommand — expected hub-up | hub-down | run-phase | record-progress",
			);
		default:
			throw new UsageError(
				`unknown subcommand "${sub}" — expected hub-up | hub-down | run-phase | record-progress`,
			);
	}
}
...
main()
	...
	.catch((err) => {
		...
		process.stderr.write(
			`meta-cc-client: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}\n`,
		);
		process.exit(1);
	});
