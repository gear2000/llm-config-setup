...
async function main() {
	...
	// ── case 6: buildPriorProgressBlock reflects passed vs not-passed + lists summaries ──
	{
		...
		recordPhaseProgress(dir, "0", "orch-plan-p0-1", "completed", "offboarded the test tenant clean\nPHASE_RESULT: passed", "hash-bbbb");
	...
	//   recorded `partial` (NOT passed) and shows under NOT PASSED on reload; only the truly-passed
	//   phase is skippable on resume. (The hub model has no guardrail breach, so the breach case from
	//   the socket ext is intentionally not exercised here — recordPhaseProgress has no breachKind.) ──
	{
		...
		// Only phase 1 (passed) is under PASSED/skip; the partial phase is under NOT PASSED.
		...
		// The partial phase sits in the NOT-PASSED region (after the NOT PASSED header), never in the
		// PASSED-skip region — so resume re-runs it and skips ONLY phase 1.
