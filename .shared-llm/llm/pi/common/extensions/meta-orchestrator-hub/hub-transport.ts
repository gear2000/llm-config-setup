...
export async function hubRunPhase(input: RunPhaseInput): Promise<PhaseOutcome> {
	...
	const teamArg = rpc.team ? "true" : "false";
	...
	writeLog(`# hub: ${input.hubJsonPath}`);
	writeLog(`# team: ${teamArg}`);
	...
	try {
		...
		events?.onLog?.("meta-orch-log", { event: "worker_up", phaseId, session, hub: input.hubJsonPath });
		const up = await runJust(repoRoot, ["worker-up", session, input.hubJsonPath, rpc.planFile, rpc.instructionsPath, rpc.resultsPath, teamArg]);
		if (up.code !== 0) {
			throw new Error(`just worker-up ${session} failed (exit=${up.code})${up.stderr ? `: ${up.stderr}` : ""}`);
		}
