...
export interface BrainState {
	...
	planHash: string;
	/** The discovery JSON path (or url) of the hub — passed to the worker's /hub-connect. */
	hubJsonPath: string;
...
export function handleRunPhase(
	state: BrainState,
	params: { phase: string; instructions: string; team: boolean },
	...
	deps.startPhase(state, deps.events, deps.pushFollowUp, deps.log, phase, instructions, params.team, iteration, phaseId);
	return {
		text: `Started phase ${phase} (iteration ${iteration}, ${phaseId}, team=${params.team}) in the background. Its completion will arrive as a follow-up message; do not call run_phase again until it does.`,
		started: true,
	};
}
...
export function startPhaseInBackground(
	...
	log({ event: "phase_start", phaseId, phase, iteration, session, team });
	hubRunPhase({
		phaseId,
		runPhaseCommand: { planFile: state.planPath, phase, instructions, instructionsPath, resultsPath, session, team },
		dirs: state.dirs,
