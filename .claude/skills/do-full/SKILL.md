...
|---|---|---|---|
| 1. Research + plan + grill | `do-plan-and-grill --route-phases <title>` | research + plan-writing = subagents | answer the grill |
| 2. Freeze → phase JSON | `rphase-create <plan.md>` | none (solo skill) | confirm any fuzzy verification |
| 3. Interactive implement | `do-loop --interactive <phases_root>` | each phase = subagent | mediate blocked phases |
...
- **`<title>`** — Required. Passed straight to `do-plan-and-grill`, which slugifies it for the work-log directory. All three stages then share that one directory.
...
Each sub-skill ends by STOPPING and telling a human to run the next command. `do-plan-and-grill` Step 7 literally says *"STOP. Do not call /rphase-create."* and, with `--route-phases`, suggests `/do-loop --afk`. **In `/do-full`, YOU are the human those messages are addressed to.**
...
- `do-plan-and-grill --route-phases` will suggest `/do-loop --afk`. **Ignore the `--afk`** — `/do-full` always goes `--interactive`.
- `rphase-create` freezes the plan and stops. Proceed straight to `do-loop --interactive`.
...
3. Stage 3 — do-loop --interactive

Mark Stage 1 `in_progress`.

### Step 1: Plan + grill (invoke do-plan-and-grill)

Invoke the **`do-plan-and-grill`** skill via the Skill tool with args: `--route-phases <title>`.
...
**Seam guard:** if the grill was abandoned and no `plan.md` was finalized (frontmatter `status` is not `Accepted`), STOP here — do not create phases from a plan that was never agreed. Tell the user the plan wasn't finalized and that they can re-run `/do-full` or pick up the draft.
...
### Step 3: Interactive implementation (invoke do-loop)

Invoke the **`do-loop`** skill via the Skill tool with args: `--interactive <plan-dir>/phases`.
...
Let `do-loop` own everything from here — phase routing, verdicts, the live-deploy gate on the final phase, results.md, decisions.md. Do not duplicate any of it.
...
> *"Plan frozen and phases created at `<plan-dir>/phases`. Continuing into implementation now. If you ever want a fresh context, you can `/clear` and run `/do-loop --interactive <plan-dir>/phases` to pick up exactly here."*
...
- **`/do-plan-and-grill`** — research + grilled plan, then STOP (you implement separately).
- **`/do-oneshot`** — lightweight research + sketch + implement, no persisted plan.
- **`/do-full`** — the whole heavyweight interactive pipeline, one command ← **you are here**.
- **`/do-loop --afk`** — unattended phased execution (the AFK path `/do-full` deliberately avoids).
