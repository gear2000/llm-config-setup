---
name: do-oneshot
description: 'Combined research + plan + implement in one shot. The lightweight shortcut through the build pipeline — for tasks too small for /do-plan-and-grill but large enough that you want some research and a quick HTML grill to shape a plan before implementing. Persists a lightweight plan.md + plan.html sketch (a record of intent, not a gate), then implements — no full plan-and-grill ceremony. Default transport is subagents (fresh context per call); pass --team to use TeamCreate (named members, TMUX windows, team-pulse heartbeat). The team leader orchestrates and delegates — does not work directly, always dispatches a subagent for concrete work. Enforces the live-deploy gate. Invocation: /do-oneshot [--team] <title>.'
---

# do-oneshot — Research + Plan + Implement, in one shot

The lightweight shortcut through the build pipeline. For tasks that need some research and a sketch of a plan but don't warrant the full `/do-plan-and-grill` ceremony with a persisted plan.md.
...
- **`/do-research`** — research only, no implementation
- **`/do-plan`** — research + persisted plan.md, no implementation
- **`/do-plan-and-grill`** — research + persisted plan + iterative grill, no implementation
- **`/do-oneshot`** — research + sketch + implement, all in one shot, no persisted plan ← **you are here**
- **`/do-implement`** — execute an existing persisted plan.md

If you find yourself wanting to revisit decisions or re-run with different scope, you've outgrown `/do-oneshot` — bail out and use `/do-plan-and-grill` instead.
...
- `plan.html` — the same sketch in the project dark style (the `<style>` block from `~/project/repos/your-repo-ops/mkdocs/docs/diagrams/architecture/v3.html` if it exists, else the default style block from the `/design-doc` skill), with `.shared-llm/public/llm/common/common/toolkits/annotation-toolkit.html` pasted verbatim immediately before `</body>`. `<title>` = `<Title> — plan v<N>`.
...
Agent(
  ...
  prompt="Watch /do-oneshot run titled <title>. Poll every 60–90s.
...
1. **Dispatch a writer subagent** to compose **two outputs** in `~/project/repos/your-repo-ops/mkdocs/docs/work-log/<YYYY-MM-DD>/<slug>/oneshot/v<N>/` — `results.md` (canonical) and `results.html` (the visual, annotatable surface you read in the browser). Same content; markdown is the agent/tooling surface, HTML is the human surface. `results.md` has the required frontmatter:
   ...
   **`results.html`** — the same content as `results.md`, rendered in the project dark style (the `<style>` block from `~/project/repos/your-repo-ops/mkdocs/docs/diagrams/architecture/v3.html` if it exists, else the default style block from the `/design-doc` skill), with `.shared-llm/public/llm/common/common/toolkits/annotation-toolkit.html` pasted verbatim immediately before `</body>`. `<title>` = `<Title> — results v<N>`. The frontmatter and `## Follow-up` discipline above govern `results.md`; the HTML twin just mirrors the rendered findings for reading and annotation.
