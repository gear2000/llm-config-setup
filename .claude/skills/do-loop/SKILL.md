...
# do-loop — In-Session Sequential Phase Loop

Run every available phase of a plan, end-to-end, inside one agent session. Each phase gets fresh context (via subagent or per-phase TeamCreate-of-one), but the *orchestrator* — you, the main thread — persists across phases. That persistence is the core value: when a phase BLOCKS, you don't have to relaunch a separate session. You research, draft options, ask the user (interactive) or escalate via code review (afk), apply the fix, and resume — all in one conversation.
...
```
/do-loop [--interactive|--afk] <phases-path-or-work-log-name> [-i N] [-t SECS] [--pulse] [--subagent]
```
...
- **`--afk`** — AFK mode. Per-phase routing by `size` tag. Mandatory code review after every PASSED worker. Single auto-fix pass on NEEDS_CHANGES before blocking. Requires a plan produced by `/do-plan-and-grill --route-phases`. **Refuses to run if any phase is missing its `size` field.**
...
Use for unattended runs after `/do-plan-and-grill --route-phases` has tagged every phase with `small` or `big`.
...
```
Cannot run --afk: phase <id> has no Size tag. Run /do-plan-and-grill
--route-phases to produce a routable plan, or invoke /do-loop
...
6. **Dispatch plan-watchdog (--afk mandatory, --interactive optional).** In `--afk` mode watchdog is always on. Spawn before the first iteration:
   ...
   Watchdog prompt:
   ```
   You are plan-watchdog for a /do-loop run.
...
TeamCreate(
  ...
  description="do-loop --interactive attempt <M> of v<N> for <phase_id>: <title>"
)
```
...
TeamCreate(
  ...
  description="do-loop --afk attempt <M> of v<N> for <phase_id>: <title>"
)
```
...
```
You are reviewing one phase of an AFK do-loop run. The worker has just
...
- **`/do-loop --interactive`**: plans you want to drive yourself, plans without size tags, plans where blocks are expected and you want conversational resolution.
- **`/do-loop --afk`**: AFK runs after `/do-plan-and-grill --route-phases`. Zero touch — kick off, walk away, come back to commits and a shortlist of blocked phases.
