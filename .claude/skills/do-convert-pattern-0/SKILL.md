---
name: do-convert-pattern-0
description: Separate a plan into work phases and Pattern 0 workflow assignments, grading each phase easy, medium or hard. Preserves draft status and unresolved choices. Preparation only, no execution or phase leaders.
argument-hint: --plan <source-plan.md> [--out <conversion-dir>] [--pool <workflow-dir>] [--non-interactive]
---
...
5. Recommend documents from the actual pool. Default bundled choices while GPT 5.6 Luna is discounted are `phase-low-Luna` for easy, `phase-medium-Luna` for medium, and `phase-high-Luna` for hard. Other bundled choices are `phase-low` for easy, `phase-medium-sonnet` for medium, and `phase-high-Astra`, `phase-high-Fable` or `phase-high-Sol` for hard. These are recommendations, not a closed registry or automatic model selection. Custom pools may use different names. Obtain human agreement on exact documents, especially medium/hard variants. Read their requirements rather than duplicating model rosters here.
