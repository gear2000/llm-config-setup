...
Prefer invoking `/upagent-offering` instead of pasting the prompt below.
...
| New **offering set** (not standard/claudex) | new `offerings.d/<set>.yaml`, extend `APPROVED_SETS` in `offerings.py`, destination `upagent.offering_sets` in `~/.shared-llm.yaml` | None |
...
**Generated (never hand-edit):** `offerings.yaml`. `just update` produces it from fragments +
...
- Pi: run `pi --list-models <search>`. The model is provider-qualified (e.g. openai-codex/gpt-6-astra).
...
2. `.shared-llm/public/extensions/common/upagent/offerings.d/standard.yaml` (or claudex.yaml / new offerings.d/<set>.yaml)
   ...
   - codex: completion_style: exec; cursor/claude/pi/claudex: interactive (or omit and use the code default)
...
2. just update  (then again; the second run must show zero changes)
3. Inspect the generated offerings.yaml and the complete diff
4. Do not run just test for a routine offering update; add focused tests only when the change crosses a scenario-specific branch
...
1. `pytest .shared-llm/public/extensions/common/upagent/offerings_test.py -q`
2. `just update` twice; the second run must report zero changes
3. Inspect the generated `offerings.yaml` and the complete diff
4. Run extra focused tests only when the change affects management, specialists, a new offering set, or harness execution policy
