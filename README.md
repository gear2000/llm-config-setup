...
3. **Pi runtime** — copies the bundled Pi extensions, agent personas, and themes into `~/.shared-llm/generated/` and symlinks them from there into `~/.pi/` (reconciling: create / re-point / prune), and scaffolds `~/.pi/agent/settings.json` from the template only if absent.
...
- `themes/rose.json` — The optional Rosé Pine Moon color theme, exposed to Pi under the short name `rose`. Symlinked into `~/.pi/agent/themes/`; on Pi 0.84.2 or newer, try it for one run with `pi --use-theme rose`, or return to the built-in theme with `pi --use-theme dark`. On older supported Pi releases, choose `rose` through `/settings`. The upstream MIT notice is retained beside it in `themes/ROSE-PINE-LICENSE.txt`.
...
```bash
just update          # deploys the OWN extensions, personas, and themes into ~/.pi/ (global step)
