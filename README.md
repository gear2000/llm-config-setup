...
- `.shared-llm/layers/llm/common/` — portable rules that apply to any project (response format, coding philosophy).
- `.shared-llm/layers/llm/this_repo/` — project-specific context: what the codebase is, how CI works, where credentials live.
- `.shared-llm/layers/skills/` — skill definitions used by the assistant as slash commands.
...
.shared-llm/
  layers/
    llm/
      common/common/response.md           — portable response-format rules (copy verbatim)
      this_repo/
        common/general.md                 — project identity, coding conventions, CI, credentials
        claude/claude.md                  — Claude Code-specific conventions (worktrees, planning)
        codex/agents.md                   — cross-harness wiring (skills, hooks, MCP)
        common/authoring.md               — example: CLAUDE.md for a special subdirectory
        common/aws-execution-engine.md    — example: CLAUDE.md for a component with a contract
        common/src/packages.md            — shared context for all package CLAUDE.mds
        common/src/services.md            — shared context for all service CLAUDE.mds
        common/packages/example_package.md — per-package leaf template
        common/services/example_service.md — per-service leaf template
    skills/
      common/python/
...
There is one install path for third-party extensions: `pi install` + the manifest. The previous `npm ci` route is gone. See `.shared-llm/llm/pi/common/THIRD-PARTY-EXTENSIONS.md` for the full set and per-extension runtime deps.
